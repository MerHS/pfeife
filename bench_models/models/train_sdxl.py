...
def parse_args(input_args=None):
    ...
    parser.add_argument("--prefetch_fwd", type=int, default=-1)
    parser.add_argument(
        "--train_data_dir",
        type=str,
        default=None,
...
def main(args):
    ...
    vae.to(net_state.device, dtype=torch.float32)
    text_encoder_one.to(net_state.device, dtype=weight_dtype)
    text_encoder_two.to(net_state.device, dtype=weight_dtype)
    ...
    progress_bar = tqdm(
        ...
        disable=not net_state.is_main_process,
    )

    # mb_size = args.train_batch_size // args.batch_cnt
    loop_cnt = args.loop_cnt if args.loop_cnt > 0 else None
    prefetch_fwd = args.prefetch_fwd if args.prefetch_fwd > 0 else None
    ...
    option = PipeOption(
        ...
        no_bench_optimizer=True,
        device_bench="profile/nccl-dev8.bench",
        # graph_bench=f"profile/sdxl-{mixed_precision}-b{mb_size}-r{args.resolution}.graph",
        # print_graph=True,
        mixed_precision=mixed_precision,
        # split_points=[232, 476, 683, 861, 1086, 1308, 1496],  # prefetch_fwd=2,
        # prefetch_fwd=2,
        # graph_bench="profile/sdxl-b1_fp32.graph",
        # verbosity="debug",
    )
    option.set_logger()
    runner = PartialRunner(option, optimizer)
    unet.to(net_state.device)
    unet = torch.compile(unet, backend=partial_compiler)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        len_data = 10000
        ti = iter(train_dataloader)
        batch = next(ti)
        for _ in range(args.batch_cnt - 1):
            b = next(ti)
            for k, v in b.items():
        ...
        batch = {
            k: (v.to(device=net_state.device) if torch.is_tensor(v) else v)
        ...
        for step in range(len_data):
            ...
            def exec_fn(batch):
                ...
                if option.mixed_precision:
                    mixed_prec_context = torch.autocast(
                        device_type="cuda", dtype=option.mixed_precision
                    )
                else:
                    mixed_prec_context = nullcontext()

                with mixed_prec_context:
                    model_input = batch["model_input"].to(net_state.device)
                    ...
                    def compute_time_ids(original_size, crops_coords_top_left):
                        ...
                        add_time_ids = torch.tensor(
                            [add_time_ids], device=net_state.device, dtype=weight_dtype
                        )
                        return add_time_ids
                    ...
                    prompt_embeds = batch["prompt_embeds"].to(
                        net_state.device, dtype=weight_dtype
                    )
            ...
            if start_time is not None and global_step % 20 == 0:
                this_time = time.time()
                throughput = global_step / (this_time - start_time)
                temp_throughput = 20 / (this_time - last_time)
                print(
                    f"step {global_step} / throughput: {temp_throughput:.5f} steps/s, total: {throughput:.5f} steps/s"
                )

                last_time = this_time

            if start_time is None and global_step == 5:
                progress_bar.refresh()
                global_step = 0

                start_time = time.time()
                last_time = start_time
                runner.log_memory_usage()
