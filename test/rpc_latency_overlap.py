...
class MyModule:
    ...
    def matmul(self, x, base_time):
        ...
        if self.next is not None:
            send = self.next.rpc_async().matmul(x, start_time)
            wait_time = time.time()
        ...
        if self.next is not None:
            send.wait()
            self.wait_time += wait_time - start_time
            self.end_time += end_time - wait_time
        else:
...
def run_main(base_time):
    print(f"Repeat 20 times, send/matmul overlap {SIZE}x{SIZE}")
    ...
    print(
        f"[Worker 2] Receive latency (1->2): {s2:8.5f}, async send: {w2:8.5f}, matmul latency: {e2:8.5f}"
    )
